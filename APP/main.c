...
//          UserPara.RotateSpeed *= 20;  //计算旋转速度  1s脉冲数*10 =10秒* 6 = 1分钟  单位：转每分
            ...
            {           
                ...
                if(Z_F_Zhuan == 1) //正转
                {
                    ...
                    {
                        UserPara.NegativeTimeBase += (UserPara.Duration + 30)/60;          //计算反转时间  +300意思是超过半分钟算1分钟
                        UserPara.Duration = 0;  //清0当前状态计时
                    }
                ...
                else               //反转
                {
                    ...
                    {
                        UserPara.PositiveTimeBase += (UserPara.Duration + 30)/60;          //计算正转时间
                        UserPara.Duration = 0;  //清0当前状态计时
                    }
                    if(UserPara.RotateSta == STA_STOP)   //上一次状态是  停转
                    {
            ...
            else     //无脉冲  停止
            {
                    if(UserPara.DirSta == Foreward)                      //上一次状态是  正转
                    {
                     ...
                     if(UserPara.DirSta == Reversal)                //上一次状态是  反转
                    {
                        UserPara.NegativeTimeBase += (UserPara.Duration + 30)/60;              //计算反转时间
                        UserPara.Duration = 0;                                      //清0当前状态计时
                    }
    //                if(UserPara.RotateSta == STA_STOP)              //上一次状态是  停转
    //                {
