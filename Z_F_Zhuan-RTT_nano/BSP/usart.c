...
//******************************************************************************
// 名称         : rt_hw_console_output()
// 创建日期     : 2020-06-09
// 作者         : weijianx
// 功能         : 实现rtthread串口rt_kprintf打印
// 输入参数     : 入：str   发送数据指针
// 输出参数     : 无
// 返回结果     : 无
// 注意和说明   : 
...
void rt_hw_console_output(const char *str)
{
    ...
    __HAL_UNLOCK(&LPUART1_HandleStruct);
    ...
    for (i = 0; i < size; i++)
    {
        if (*(str + i) == '\n')
        {
            HAL_UART_Transmit(&LPUART1_HandleStruct, (uint8_t *)&a, 1, 1);
        }
        HAL_UART_Transmit(&LPUART1_HandleStruct, (uint8_t *)(str + i), 1, 1);
    }
}
...
{  //查询方式实现，记得将USART1初始化中的中断接收配置相关代码注释掉
	 int ch = -1;
	   /*等待串口1输入数据*/
     if(__HAL_UART_GET_FLAG(&LPUART1_HandleStruct, UART_FLAG_RXNE) != RESET)
	 {

		 HAL_UART_Receive(&LPUART1_HandleStruct, (uint8_t*)ch, sizeof(ch), 1000);
		 __HAL_UART_CLEAR_FLAG(&LPUART1_HandleStruct, UART_FLAG_RXNE);
	 }
	 else
	 {
		 
		 if(__HAL_UART_GET_FLAG(&LPUART1_HandleStruct, UART_FLAG_ORE) != RESET)
		 {
//			 USART_ClearFlag(USART1, USART_FLAG_ORE);
			 __HAL_UART_CLEAR_FLAG(&LPUART1_HandleStruct, UART_FLAG_ORE);
		 }
		 rt_thread_mdelay(10);
